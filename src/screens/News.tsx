...
const NewsScreen = ({ getAllNews, route }: ReduxProps & any) => {
    const { userData } = route.params;
    const navigation = useNavigation();
    const [allNewsData, setAllNewsData] = useState([]);

    const fetchData = async () => {
        const getAllNewsApi = await getAllNews();
        const getAllNewsRes = getAllNewsApi?.payload?.data?.news;
...
const NewsScreenRedux = connector(NewsScreen);

export { NewsScreenRedux as NewsScreen };